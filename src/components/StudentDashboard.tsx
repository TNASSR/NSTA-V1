...
import { Zap, Crown, Calendar, Clock, History, Layout, Gift, Sparkles, Megaphone, Lock, BookOpen, AlertCircle, Edit, Settings, Play, Pause, RotateCcw, MessageCircle, Gamepad2, Timer, CreditCard, Send, CheckCircle, Mail, X, Ban, Smartphone, Link as LinkIcon, ExternalLink } from 'lucide-react';
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings }) => {
  ...
  return (
    <div>
        ...
        <div className={`grid ${isGameEnabled ? 'grid-cols-5' : 'grid-cols-4'} gap-1 bg-white p-2 rounded-xl border border-slate-200 shadow-sm mb-8 sticky top-20 z-20`}>
            ...
            <button onClick={() => setActiveTab('REDEEM')} className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all ${activeTab === 'REDEEM' ? 'bg-slate-100 text-[var(--primary)]' : 'text-slate-400 hover:bg-slate-50'}`}><LinkIcon size={18} className="mb-1" /><span className="text-[9px] font-bold uppercase">Links</span></button>
        </div>
            ...
            {activeTab === 'REDEEM' && (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="bg-slate-900 rounded-2xl p-6 text-center text-white mb-8">
                        ...
                        <p className="text-slate-400 text-sm">Access official links and extra materials provided by the Admin.</p>
                    </div>
                    {(!settings?.vercelLinks || settings.vercelLinks.length === 0) ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {settings.vercelLinks.map(link => (
                                <a
                                    key={link.id}
                                    href={link.url}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="block bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:border-[var(--primary)] hover:shadow-lg transition-all group"
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <h3 className="font-bold text-lg text-slate-800">{link.name}</h3>
                                        </div>
                                        <ExternalLink size={20} className="text-slate-400 group-hover:text-[var(--primary)]" />
                                    </div>
                                    <p className="text-xs text-slate-500 mt-2 truncate">{link.url}</p>
                                </a>
                            ))}
                        </div>
                    )}
