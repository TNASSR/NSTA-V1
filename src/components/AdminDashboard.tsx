...
export const AdminDashboard: React.FC<Props> = ({ onNavigate, settings, onUpdateSettings, onImpersonate }) => {
  ...
  const [localSettings, setLocalSettings] = useState<SystemSettings>(settings || {
      ...
      customCSS: '',
      apiKeys: ['AIzaSy... (Paste Your Key Here)'],
      chatCost: 1,
      dailyReward: 3,
      signupBonus: 2,
      isChatEnabled: true,
  ...
  const [selectedChatUser, setSelectedChatUser] = useState<User | null>(null); // For Private Admin Chat
  ...
  const handleTestKey = async (key: string) => {
      ...
      try {
          ...
          if (result.response.text()) alert("✅ CONNECTION SUCCESSFUL!");
  ...
  if (showChat) {
      return (
          <div className="animate-in fade-in slide-in-from-right duration-500 flex flex-col h-[85vh]">
              <div className="flex justify-between items-center mb-4">
                  <button onClick={() => { setShowChat(false); setSelectedChatUser(null); loadData(); }} className="text-blue-600 font-bold hover:underline flex items-center gap-1">&larr; Back to Dashboard</button>
                  <div className="flex bg-slate-100 rounded-lg p-1 gap-1">
                      <button
                          onClick={() => setSelectedChatUser(null)}
                          className={`px-3 py-1 text-xs font-bold rounded ${!selectedChatUser ? 'bg-white shadow text-blue-600' : 'text-slate-500'}`}
                      >
                          Global Group
                      </button>
                      <button
                          onClick={() => alert("Select a user from the 'Users' tab to message privately.")}
                          className={`px-3 py-1 text-xs font-bold rounded ${selectedChatUser ? 'bg-white shadow text-blue-600' : 'text-slate-500'}`}
                      >
                          {selectedChatUser ? `Chat with ${selectedChatUser.name.split(' ')[0]}` : 'Private Inbox'}
                      </button>
                  </div>
              </div>

              {selectedChatUser ? (
                  // Private Chat View (Placeholder or Actual Logic)
                  <div className="flex-1 border rounded-2xl p-10 flex flex-col items-center justify-center bg-slate-50">
                      <MessageCircle size={48} className="text-slate-300 mb-4" />
                      <h3 className="text-lg font-bold text-slate-700">Private Chat with {selectedChatUser.name}</h3>
                      <p className="text-slate-400 text-sm mb-6">This feature connects to UniversalChat with a specific filter.</p>
                      <button onClick={() => setSelectedChatUser(null)} className="text-blue-600 font-bold text-sm">Return to Group</button>
                  </div>
              ) : (
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="space-y-6">
                        ...
                        <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl space-y-3">
                            <div className="flex justify-between items-center">
                                <div>
                                    <div className="font-bold text-blue-900">API Key Manager</div>
                                    <div className="text-[10px] text-blue-600">Get key from: aistudio.google.com</div>
                                </div>
                                <div className="bg-blue-200 text-blue-800 px-2 py-1 rounded text-xs font-bold">{localSettings.apiKeys?.length || 0} Keys Active</div>
                            </div>
                            <div className="flex gap-2">
                                <input type="text" placeholder="Paste Gemini API Key" value={newApiKey} onChange={e => setNewApiKey(e.target.value)} className="flex-1 px-3 py-2 border rounded-lg text-sm" />
                                <button onClick={handleAddKey} className="bg-blue-600 text-white px-3 py-2 rounded-lg font-bold text-xs hover:bg-blue-700">Add</button>
                            </div>
                            <div className="max-h-32 overflow-y-auto space-y-1">
                                {localSettings.apiKeys?.map((k, idx) => (
                                    <div key={idx} className="flex justify-between items-center bg-white p-2 rounded border border-blue-100 text-xs">
                                        <span className={`font-mono ${k.startsWith('AIza') ? 'text-green-600 font-bold' : 'text-slate-500'}`}>{k.substring(0, 12)}...</span>
                                        <div className="flex gap-1">
                                            <button onClick={() => handleTestKey(k)} disabled={testingKey} className="text-blue-500 bg-blue-50 p-1 rounded">Test</button>
                                            <button onClick={() => handleRemoveKey(idx)} className="text-red-500 p-1"><Trash2 size={14} /></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                   ...
                   <div className="space-y-4">
                       ...
                       <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                           <div className="font-bold text-slate-700 mb-2">Chat Mode</div>
                           <div className="flex bg-white rounded-lg p-1 border border-slate-200">
                               {['PRIVATE_ONLY', 'UNIVERSAL_ONLY', 'BOTH'].map((mode) => (
                                   <button
                                       key={mode}
                                       onClick={() => setLocalSettings({...localSettings, chat_mode: mode as any})}
                                       className={`flex-1 py-2 text-[10px] font-bold rounded-md transition-all ${localSettings.chat_mode === mode ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}
                                   >
                                       {mode.replace('_', ' ')}
                                   </button>
                               ))}
