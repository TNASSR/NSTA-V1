...
import { FULL_SYLLABUS } from "../data/fullSyllabus";

// ============================================================================
// 🔑 MANUAL BACKUP KEY - REMOVED TO PREVENT LEAK ERRORS
    ...
    try {
        ...
        if (storedSettings) {
            ...
            if (parsed.apiKeys && Array.isArray(parsed.apiKeys)) {
                parsed.apiKeys.forEach(k => { if(k.trim() && !k.includes('Paste Your Key')) keys.push(k.trim()); });
            }
        }
...
const executeWithRotation = async <T>(
    ...
    if (shuffledKeys.length === 0) {
        throw new Error("No API Keys available. Please add a valid Gemini API Key in Admin Dashboard > System.");
    }
...
> **Note:** You are viewing **Offline Notes**. The AI Server is currently unreachable (No Valid API Key).
    ...
    try {
        return await executeWithRotation(async (ai) => {
            const model = ai.getGenerativeModel({ 
                ...
                systemInstruction: "You are a React Developer. Output only code." 
            });
            const result = await model.generateContent(userPrompt);
            return result.response.text() || "// Error generating code";
        });
    } catch (error) {
...
export const fetchChapters = async (
  ...
  // 3. CHECK STATIC FULL SYLLABUS DATASET
  // Construct lookup key: e.g., "CBSE-10-Science" or "CBSE-11-Science-Physics"
  let syllabusLookupKey = `${board}-${classLevel}-${subject.name}`;
  if (stream) {
      syllabusLookupKey = `${board}-${classLevel}-${stream}-${subject.name}`;
  }
  
  const staticSyllabus = FULL_SYLLABUS[syllabusLookupKey] || FULL_SYLLABUS["GENERIC"]; // Fallback to GENERIC if specific missing
  
  if (staticSyllabus) {
      console.log(`Loaded Chapters from Static Dataset: ${syllabusLookupKey}`);
      const chapters: Chapter[] = staticSyllabus.chapters.map((item, index) => ({
          id: `ch-static-${index + 1}`,
          title: item.title,
  ...
  // 4. ATTEMPT AI GENERATION (Only if Keys exist)
  try {
    ...
    console.error("Fetch Chapters Failed (All keys exhausted or invalid), using Offline Fallback:", error);
    // Even if static missing and AI fails, use hardcoded backup
...
export const fetchLessonContent = async (
  ...
  // USE STABLE MODELS
  // As of late 2025, Gemini 1.5 might be legacy. Using auto-latest aliases is safer.
  // Or check if 2.0 is available. For now, we stick to the strings provided by Google AI Studio
  // which might be 'gemini-1.5-flash-latest' or 'gemini-2.0-flash-exp'.
  // However, the error 404 suggests the endpoint changed. 
  // Let's use 'gemini-1.5-flash-latest' which is often an alias that works longer, 
  // OR simply 'gemini-pro' which is the safest fallback if 1.5 is gone from v1beta.
  
  const modelToUse = (type.includes('PREMIUM') || type === 'MCQ_ANALYSIS' || isPremium) ? 'gemini-1.5-pro' : 'gemini-1.5-flash';
  ...
  try {
    const text = await executeWithRotation(async (ai) => {
        ...
        if (!txt) throw new Error("No response text");
        return txt;
    });
